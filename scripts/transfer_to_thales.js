...
async function main() {
  ...
  const recipient = "0xb2D0fe4C5B4f8D2bb0B29ffF97db6b485B1F4e55"; // Substitua pelo endereço que receberá os tokens
  const amount = hre.ethers.parseUnits("100", 18); // 1000 tokens (ajuste se necessário)

  const tx = await BrunoCoin.transferTo(deployer.address, recipient, amount)
